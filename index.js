  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      if (window.solana && window.solana.isPhantom) {
        ...
        state.wallet.address = shortAddr;
        state.wallet.balance = "0.00"; // Requires RPC to fetch real balance
        
        btn.innerHTML = `<span class="wallet-dot"></span>${shortAddr} | ${state.wallet.balance} SOL`;
        ...
        btn.innerHTML = `<span class="wallet-dot"></span>${shortAddr} | ETH`;
      } else {
        showNotification("⚠️ Please install Phantom (Solana) or MetaMask (Ethereum) extension!");
      }
    } catch (err) {
      console.error("Wallet connection error:", err);
      showNotification("❌ Wallet connection failed or canceled");
    }
  });
}
